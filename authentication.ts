...
export class AuthenticationClient {
  ...
  protected async post(
    ...
    const resp = await fetch(this.host + "/" + RootPath + "/" + endpoint, {
      method: "POST",
  ...
  authenticate(scopes: string[], force: boolean): Promise<ITwoLeggedToken> {
    ...
    const cache = this._cached[key] = {
      ...
      promise: this.post("authenticate", params)
        .then((resp) => resp.json())
        .then((data) => {
  ...
  getAuthorizeRedirect(scopes: string[], redirectUri: string): string {
    ...
    return `${this.host}/${RootPath}/authorize?${stringify(params)}`;
  }
  ...
  async getUserProfile(token: string): Promise<IUserProfile> {
    ...
    const resp = await fetch(this.host + "/userprofile/v1/users/@me", config);
    const data = await resp.json();
    return data;
  }
}
